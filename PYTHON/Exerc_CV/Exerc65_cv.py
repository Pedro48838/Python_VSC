...
while resp in 'Ss':
    ...
    if quant == 1:
        ...
        if num > maior:
            maior = num
        if num < menor:  # Com dois if separados, ambos os blocos condicionais serão avaliados independentemente.
            menor = num
    resp = str(input("Quer continuar?  [S/N] ")).upper().strip()[0]  #para pegar somente o primeiro caracter
    media = soma / quant
